...
public class GunRay : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Debug.DrawLine(camTr.position, shotHit.point);
    ...
    void CheckScope()
    {
        ...
        if (Physics.Raycast(FPivot.transform.position, (FPivot.transform.position - BPivot.transform.position).normalized, out shotHit, 1000f))
        {
            print(shotHit.point);
            Debug.DrawLine(FPivot.transform.position, shotHit.point);
        }
        if (Physics.Raycast(camTr.position, (shotHit.point-camTr.transform.position).normalized, out scopeHit, 1000f, layerMaskBPivot) && shotHit.point != null)
        {
            Debug.Log("hit");
